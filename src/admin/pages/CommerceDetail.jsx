...
import '../styles/CommerceDetail.scss';
...
function CommerceDetail() {
  ...
  const fetchCommerceDetails = async () => {
    try {
      ...
      // Récupérer les détails du commerce
      const commerceResponse = await axios.get(`${ API_BASE_URL }/api/commerces/${id}`);
      setCommerce(commerceResponse.data);

      // Récupérer les sections
      const sectionsResponse = await axios.get(`${ API_BASE_URL }/api/commerces/${id}/sections`);
      setSections(sectionsResponse.data);

      // Récupérer la galerie
      const galerieResponse = await axios.get(`${ API_BASE_URL }/api/commerces/${id}/galerie`);
      setGalerie(galerieResponse.data);

      setLoading(false);
    } catch (err) {
      console.error('Erreur lors du chargement des détails du commerce:', err);
      setError('Erreur lors du chargement des détails du commerce');
      setLoading(false);
    }
  };
  ...
  const handleSectionSubmit = async (e) => {
    ...
    try {
      ...
      if (sectionFormData.imageFile) {
        ...
        const uploadResponse = await axios.post(`${ API_BASE_URL }/api/upload/commerce-section/${id}`, formDataFile);
      ...
      if (currentSection) {
        ...
        await axios.put(`${ API_BASE_URL }/api/commerce-sections/${currentSection.id}`, sectionData);
      } else {
        // Création d'une nouvelle section
        await axios.post(`${ API_BASE_URL }/api/commerces/${id}/sections`, sectionData);
      }
  ...
  const handleDeleteSection = async (sectionId) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer cette section ?')) {
      try {
        await axios.delete(`${ API_BASE_URL }/api/commerce-sections/${sectionId}`);
        fetchCommerceDetails();
      } catch (err) {
  ...
  const handleGalerieSubmit = async (e) => {
    ...
    try {
      ...
      // Télécharger l'image dans le dossier du commerce
      const formDataFile = new FormData();
      formDataFile.append('image', galerieFormData.imageFile);
      const uploadResponse = await axios.post(`${ API_BASE_URL }/api/upload/commerce-galerie/${id}`, formDataFile);
      const imageUrl = uploadResponse.data.imageUrl;

      // Ajouter l'image à la galerie
      await axios.post(`${ API_BASE_URL }/api/commerces/${id}/galerie`, {
        imageUrl,
  ...
  const handleDeleteGalerieImage = async (imageId) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer cette image ?')) {
      try {
        await axios.delete(`${ API_BASE_URL }/api/commerce-galerie/${imageId}`);
        fetchCommerceDetails();
      } catch (err) {
  ...
  const handleBack = () => {
    navigate('/adminfermebossejauque/Commerces');
  };

  if (loading) return <div className="loading">Chargement...</div>;
  if (error) return <div className="error">{error}</div>;
  if (!commerce) return <div className="error">Commerce non trouvé</div>;
    ...
    <div className="commerce-detail">
      ...
      <div className="commerce-info">
        <div className="commerce-image">
          <img src={commerce.imageUrl || '/assets/images/placeholder.jpg'} alt={commerce.titre} />
        </div>
        <div className="commerce-meta">
          <p><strong>Description:</strong> {commerce.description}</p>
          <p><strong>Type:</strong> {commerce.type}</p>
          {commerce.telephone && <p><strong>Téléphone:</strong> {commerce.telephone}</p>}
          <p><strong>Date de création:</strong> {new Date(commerce.datecreation).toLocaleDateString()}</p>
        </div>
      </div>
      ...
      {activeTab === 'sections' && (
        <div className="commerce-sections">
          ...
          <div className="sections-list">
            {sections.length === 0 ? (
              <p>Aucune section trouvée pour ce commerce.</p>
            ) : (
